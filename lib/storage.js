...
function _doTableRequest (params, errFunc, doneFunc, rowFunc, continuePartition, continueRow) { 
   ...
   if (continuePartition !== null) {
     // append continuation to fullParams.uri
   }
        ...
        p.addListener('endElement', function(name) {
          if (name === "entry") {
            ...
            if (response.headers["x-ms-continuation-NextPartitionKey"] != null) {
              _doTableRequest(params, rowFunc, doneFunc, errFunc, response.headers["x-ms-continuation-NextPartitionKey"], response.headers["x-ms-continuation-NextRowKey"])
            } else {
              doneFunc();
            }
          }
