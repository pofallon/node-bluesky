...
function _doTableRequest (params, errFunc, doneFunc, rowFunc, continuePartition, continueRow) { 
   ...
   request(fullParams, function(error, response, body) {
        ...
        p.addListener('endElement', function(name) {
          if (name === "entry") {
            ...
            if (response.headers["x-ms-continuation-nextpartitionkey"] !== undefined) {
              _doTableRequest(params, errFunc, doneFunc, rowFunc, response.headers["x-ms-continuation-nextpartitionkey"], response.headers["x-ms-continuation-nextrowkey"])
            } else {
              doneFunc();
            }
          }
...
var buildAuthorization = function(verb, ts, acct, key, path, md5, ctype) {
  ...
  // console.log("Old key: " + base64_decode(key) + ", New key: " + decodedKey.toString('utf8'));
  var stringToSign = verb + "\n" + md5 + "\n" + ctype + "\n" + ts + "\n" + canonicalizedResource;
  var signature = crypto.createHmac('sha256',base64_decode(key)).update(stringToSign).digest('base64');
  // var signature = crypto.createHmac('sha256',decodedKey.toString()).update(stringToSign).digest('base64');
