...
Table.listTables = function(options,callback) {
  ...
  azureutil.request.get(
    { uri : 'http://' + options.host + '/' + path,
      headers : {
        'x-ms-date' : now,
        'Date' : now,
        'Authorization' : azureutil.buildAuthorization('GET',now,options.account,options.key,path, '', '')
      }
    }).pipe(saxStream);
};
  ...
  azureutil.request.post(
    { uri : 'http://' + options.host + '/' + path,
      ...
      headers : {
        ...
        'Content-MD5' : md5,
        'Authorization' : azureutil.buildAuthorization('POST',now,options.account,options.key,path,md5,contentType)
      },
  ...
  azureutil.request.del(
    { uri : 'http://' + options.host + '/' + path,
      headers : {
        ...
        'Content-Type' : contentType,
        'Content-Length' : 0,
        'Authorization' : azureutil.buildAuthorization('DELETE',now,options.account,options.key,path, '', contentType)
      }
